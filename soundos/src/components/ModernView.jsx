...
export default function ModernView(props) {
  ...
  const { currentTrack, isPlaying, progress } = audio
  ...
  return (
    <div className="bl-root">
      {currentTrack?.cover && (
        <div className="bl-bg-art" style={{ backgroundImage: `url(${currentTrack.cover})` }} />
      )}
    ...
    <ul className="bl-menu">
      {items.map((item, i) => (
        <li key={item.id}
          ...
          onClick={onSelect}
          onMouseEnter={() => { const d = i - selectedIndex; if (d !== 0) onScroll(d) }}
        >
  ...
  return (
    <div className="bl-np">
      <div className="bl-np-art">
        {currentTrack.cover ? <img src={currentTrack.cover} alt="cover" /> : <div className="bl-np-art-ph">♪</div>}
      </div>
      ...
      <div className="bl-np-bar" onClick={e => { const r = e.currentTarget.getBoundingClientRect(); seek(((e.clientX - r.left) / r.width) * 100) }}>
  ...
  return (
    <div className="bl-settings">
      ...
      <div className="bl-settings-item bl-settings-mode-row">
        <span className="bl-settings-mode-label">View Mode</span>
