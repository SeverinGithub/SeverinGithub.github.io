...
// ─────────────────────────────────────────────────────────────
const HomeScreen = ({ mode, onNav }) => {
  const todayOutfit = OUTFITS[0];
  const todayItems = todayOutfit.items.map(id => ITEMS_BY_ID[id]);
  ...
  const stats = {
    ...
    worn: 18,
    unworn: 6,
    mostWorn: [...ITEMS].sort((a,b)=>b.worn-a.worn)[0],
  };
    ...
    <div className="fade-in" style={{ paddingBottom: 110 }}>
      ...
      <div style={{ padding: '20px 20px 26px', borderTop: '0.5px solid var(--line)', borderBottom: '0.5px solid var(--line)', background: 'var(--surface)' }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={() => onNav('outfit', todayOutfit.id)} className="press" style={{
            flex: 1, padding: '12px', background: 'var(--ink)', color: 'var(--bg)',
        ...
        <div className="no-scrollbar" style={{ display: 'flex', gap: 10, overflowX: 'auto', padding: '0 20px 4px' }}>
          {suggestions.map(o => (
            <div key={o.id} style={{ flex: '0 0 160px' }}>
              <OutfitCard outfit={o} onClick={() => onNav('outfit', o.id)} mode={mode}/>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ paddingTop: 32 }}>
        <SectionHead eyebrow="Wardrobe · 28 New" title="Recently added" action="Browse" onAction={() => onNav('wardrobe')}/>
      ...
      <div style={{ padding: '32px 20px 0' }}>
        ...
        <div style={{
          ...
          <StatTile label="Days worn"    value="22"  hint="of 31"/>
          <StatTile label="Most worn"    value={stats.mostWorn.worn.toString()} hint={stats.mostWorn.name}/>
...
// 7-day strip showing recent worn outfits
