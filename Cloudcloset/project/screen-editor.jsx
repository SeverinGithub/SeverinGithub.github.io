// ─────────────────────────────────────────────────────────────
// EDITOR screen — add new item, multi-step flow
// ─────────────────────────────────────────────────────────────
const EditorScreen = ({ mode, onNav }) => {
  const [step, setStep] = React.useState(0); // 0 capture, 1 process, 2 tag, 3 done
  ...
  return (
    <div className="fade-in" style={{ paddingBottom: 110 }}>
      ...
      {step === 0 && <Step0Capture onNext={(d) => { setDraft({ ...draft, ...d }); setStep(1); }} mode={mode}/>}
      {step === 1 && <Step1Process draft={draft} setDraft={setDraft} onNext={() => setStep(2)} onBack={() => setStep(0)}/>}
      {step === 2 && <Step2Tag draft={draft} setDraft={setDraft} onDone={() => onNav('home')} onBack={() => setStep(1)}/>}
    </div>
  );
};

// ─────────────────────────────────────────────────────────────
const Step0Capture = ({ onNext, mode }) => {
  // simulated "recent photos" rail
  const recents = [
      ...
      <div style={{ padding: '0 20px 16px' }}>
        <button onClick={() => onNext({ tone: '#5C7796', pat: 'denim' })} className="press" style={{
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{
              fontFamily: 'var(--mono)', fontSize: 10, letterSpacing: '0.18em', opacity: 0.7,
...
// ─────────────────────────────────────────────────────────────
const Step1Process = ({ draft, setDraft, onNext, onBack }) => {
...
// ─────────────────────────────────────────────────────────────
const Step2Tag = ({ draft, setDraft, onDone, onBack }) => {
  ...
  const [name, setName] = React.useState('Vintage Denim Jacket');
    ...
    <div style={{ padding: '0 20px' }}>
      ...
      <FieldBlock label="Category">
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
          {cats.map(c => (
            <Chip key={c} mono active={draft.cat === c} onClick={() => setDraft({...draft, cat: c})}>{c}</Chip>
          ))}
        </div>
      </FieldBlock>
      ...
      <FieldBlock label="Color">
        <div style={{ display: 'flex', gap: 8 }}>
          {tones.map(c => (
            <button key={c} onClick={() => setDraft({...draft, tone: c})} className="press" style={{
      ...
      <FieldBlock label="Season">
        <div style={{ display: 'flex', gap: 6 }}>
          {seasons.map(s => {
            ...
            return (
              <Chip key={s} active={on} onClick={() => {
                setDraft({...draft, season: on ? draft.season.filter(x => x !== s) : [...draft.season, s]});
              }}>{s}</Chip>
            );
          })}
        </div>
      ...
      <FieldBlock label="Tags">
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
          {['casual','vintage','everyday','spring','denim','blue'].map(t => (
            <Chip key={t} active={true}># {t}</Chip>
          ))}
          <Chip><Icon name="plus" size={11} sw={1.6}/> add</Chip>
        </div>
      </FieldBlock>
      ...
      <FieldBlock label="Purchase · optional">
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          <DetailField label="Brand"  placeholder="Levi's"/>
          <DetailField label="Price"  placeholder="€ 89"/>
        </div>
      </FieldBlock>
      ...
      <div style={{ display: 'flex', gap: 8, marginTop: 26 }}>
        ...
        <button onClick={onDone} className="press" style={{
          ...
          <Icon name="check" size={14} sw={1.8}/> Save to closet
        </button>
      </div>
      ...
      <div style={{
        ...
        We'll suggest 3 outfits using this piece.
      </div>
    </div>
  );
};
...
const DetailField = ({ label, placeholder }) => (
